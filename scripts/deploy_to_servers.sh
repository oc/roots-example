...
servers=( "node1" "node2" )
artifacts=("../jetty-pkg/target/roots-$version.zip")
config_dir="../config"
...
do
  server_host="$server$server_suffix"
	echo "Deploying roots on $server_host"
	ssh -tt roots@$server_host "cd /srv/roots; nohup ./deploy.sh roots $version > /dev/null 2>&1 </dev/null"
	if [ "$?" -ne "0" ]; then
		echo "deploy.sh for roots over ssh failed! Quitting..."
		exit 127
	fi
done
